...
from prepro import get_dataset_and_examples
...
def main():
    ...
    #         examples[question_column_name if pad_on_right else context_column_name], # default question
    #         examples[context_column_name if pad_on_right else question_column_name], # default context
    ...
    if training_args.do_train:
        ...
        #     train_dataset = train_dataset.select(range(data_args.max_train_samples))
        train_dataset, train_examples = get_dataset_and_examples('train', tokenizer, data_args)
    ...
    if training_args.do_eval:
        eval_dataset, eval_examples = get_dataset_and_examples('dev', tokenizer, data_args)
    ...
    if training_args.do_predict:
        predict_dataset, predict_examples = get_dataset_and_examples('test', tokenizer, data_args)
    ...
    if training_args.local_rank != -1:
        print('process %d loading datasets and model completed...'%(dist.get_rank()))
        dist.barrier()
    ...
    metric = load_metric("squad_v2/squad_v2.py" if data_args.version_2_with_negative else "squad")
