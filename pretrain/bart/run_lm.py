...
def main():
    ...
    weight_np = model.model.shared.weight.detach().numpy()
    mean = np.mean(weight_np, axis=0)
    cov = np.cov(weight_np, rowvar=False)
    pos_weight_np = model.model.encoder.disc_connect_embedding.weight.detach().numpy()
    new_weight = np.random.multivariate_normal(mean, cov, pos_weight_np.shape[0])
    model.model.encoder.disc_connect_embedding = nn.Embedding.from_pretrained(torch.Tensor(new_weight))
    model.model.encoder.disc_connect_embedding.weight.requires_grad = True

    if training_args.update_mode == 'partial':
        for p in model.named_parameters():
            if ('disc' not in p[0]) and ('lm_head' not in p[0]):
    ...
    if training_args.do_eval:
        dev_dataset = LMDataset('dev', data_args,tokenizer)
    ...
    if training_args.do_predict:
        test_dataset = LMDataset('test',  data_args,tokenizer)
        # max_target_length = data_args.val_max_target_length
