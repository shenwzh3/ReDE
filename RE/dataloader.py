...
def load_vocab(args = None):
    label_vocab = pickle.load(open(args.data_dir + '/DialogRE_v1/label_vocab.pkl', 'rb'))

    # return speaker_vocab, label_vocab, discourse_rel_vocab
    return label_vocab

def get_DialogRE_loaders_DDP(batch_size=32, num_workers=0, pin_memory=False, args = None):
    # distributed data loader
    # tokenizer = RobertaTokenizer.from_pretrained(args.home_dir + args.bert_tokenizer_dir)
    ...
    trainset = REDataset('train', label_vocab, args, tokenizer)
    ...
    devset = REDataset('dev', label_vocab, args, tokenizer)
    ...
    testset = REDataset('test', label_vocab, args, tokenizer)
...
def get_DialogRE_loaders(batch_size=32, num_workers=0, pin_memory=False, args = None):
    if args.local_rank != -1:
       return get_DialogRE_loaders_DDP(batch_size=batch_size, num_workers=0, args = args)
    else:
        tokenizer = RobertaTokenizer.from_pretrained(args.bert_tokenizer_dir)
        print('building vocab.. ')
        label_vocab = load_vocab(args)
