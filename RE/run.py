...
if __name__ == '__main__':
    ...
    parser.add_argument('--bert_model_dir', type=str, default='models/roberta-base_disc/')
    parser.add_argument('--bert_tokenizer_dir', type=str, default='models/roberta-base_disc/')
    ...
    parser.add_argument('--max_sent_len', type=int, default=400,
    ...
    parser.add_argument('--max_hop', type=int, default=100,
    ...
    train_loader, valid_loader, test_loader, label_vocab, train_size, dev_size, test_size = get_DialogRE_loaders(
        batch_size=batch_size, num_workers=0, args=args)
    if args.local_rank == -1 or args.local_rank == 0:
    ...
    # logger = get_logger('/data/SHENWZ/ERC/outs/bert/'+args.dataset_name+'/bert_win15_base_2.log')
    # logger.info('start training on GPU {}!'.format(os.environ["CUDA_VISIBLE_DEVICES"]))
    # logger.info(args)
    # PATH = '/data1/chenjunqing/ERC/trained_models/bert/'+args.dataset_name+'/'
    cur_best_on_valid = 0.0
    for e in range(n_epochs):
        start_time = time.time()
        if args.local_rank == -1:
            train_loss, train_precision, train_recall, train_f1, _, _ = train_model(model, loss_function, train_loader,
                                                                                    e, cuda, args, optimizer)
            valid_loss, valid_precision, valid_recall, valid_f1, _, _ = eval_model(model, loss_function, valid_loader,
                                                                                   e, cuda, args)
            test_loss, test_precision, test_recall, test_f1, test_label, test_pred = eval_model(model, loss_function,
                                                                                                test_loader, e, cuda,
                                                                                                args)
        else:
            train_loss, train_precision, train_recall, train_f1, _, _ = train_model_DDP(model, loss_function,
                                                                                        train_loader, e, cuda, args,
                                                                                        train_size, optimizer)
            valid_loss, valid_precision, valid_recall, valid_f1, _, _ = eval_model_DDP(model, loss_function,
                                                                                       valid_loader, e, cuda, dev_size,
                                                                                       args)
            test_loss, test_precision, test_recall, test_f1, test_label, test_pred = eval_model_DDP(model,
                                                                                                    loss_function,
                                                                                                    test_loader, e,
                                                                                                    cuda, test_size,
                                                                                                    args)
        # train_loss, train_acc, _, _, train_fscore = eval_model(model, loss_function,train_loader, e, cuda,args)
        # if best_acc< test_acc:
        #     best_acc=test_acc
        #     best_fscore, best_loss, best_label, best_pred = test_fscore,test_loss,test_label,test_pred
        #     if best_acc>60. :
        #         torch.save(model.state_dict(), PATH + 'Bert_' + str(e) + '_' + str(best_acc)+ '.pth')

        all_fscore.append([valid_f1, test_f1, test_precision, test_recall])
        ...
        #     best_fscore, best_loss, best_label, best_pred = test_fscore,test_loss,test_label,test_pred
        #     if best_acc>65. :
        #         torch.save(model.state_dict(), StorePATH + args.basemodel+'_' + str(e) + '_' + str(best_acc)+ '.pth')
        #         logger.info('model {} saved'.format(StorePATH + args.basemodel+'_' + str(e) + '_' + str(best_acc)+ '.pth'))
        ...
        if args.local_rank == -1 or args.local_rank == 0:
            if valid_f1 > cur_best_on_valid:
                cur_best_on_valid = valid_f1
                torch.save({'args': args, 'model_state_dict': model.state_dict()}, 'optimal_model.pkl')
            print(
                'epoch: {},train_loss: {},  train_recall: {}, train_precision: {}, train_fscore: {}, valid_loss: {},  valid_recall: {}, valid_precision: {},  valid_fscore: {}, test_loss: {}, test_recall: {}, test_precision: {},  test_fscore: {}, time: {} sec'. \
                format(e + 1, train_loss, train_recall, train_precision, train_f1, valid_loss, valid_recall,
                       valid_precision, valid_f1, test_loss, test_recall, test_precision, test_f1,
    ...
    if args.local_rank == -1 or args.local_rank == 0:
        ...
        print('Best F-Score based on validation:', all_fscore[0][1], 'percision:', all_fscore[0][2], 'recall:',
              all_fscore[0][3])
        print('Best F-Score based on test:', max([f[1] for f in all_fscore]))

        with open('results.txt', 'a') as f:
            f.write('Test performance..\n' + 'Best F-Score based on validation: ' + str(
                all_fscore[0][1]) + ' percision: ' + str(all_fscore[0][2]) + ' recall: ' + str(
